...
namespace
{
    ...
    class CSTFunction : public geoml::MathFunc3d
    {
    public:
        CSTFunction(geoml::CCSTCurveBuilder* b)
            : builder(b)
        {
        }
        ...
        double valueY(double t) override
        {
            ...
            return geoml::cstcurve(builder->N1(), builder->N2(), builder->B(), builder->T(), x);
        }
    ...
    private:
        geoml::CCSTCurveBuilder* builder;
    };
}
...
Handle(Geom_BSplineCurve) CCSTCurveBuilder::Curve()
{
    ...
    if (_algo == Algorithm::Piecewise_Chebychev_Approximation)
    {
        ...
        throw geoml::Error("Unknown algorithm enum value passed to CCSTCurveBuilder", GENERIC_ERROR);
    }
}
