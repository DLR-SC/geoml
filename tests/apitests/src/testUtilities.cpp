...
EXPECT_NEAR(getPrimal(geoml::extract_control_point_vector_in_U_direction(test_surface,0).at(2).Y()), 2., 1e-5);
EXPECT_NEAR(getPrimal(geoml::extract_control_point_vector_in_U_direction(test_surface,1).at(2).Y()), 2., 1e-5);
EXPECT_NEAR(getPrimal(geoml::extract_control_point_vector_in_V_direction(test_surface,0).at(1).Y()), 0., 1e-5);
EXPECT_NEAR(getPrimal(geoml::extract_control_point_vector_in_U_direction(test_surface,1).at(0).Y()), 0., 1e-5);
...
{  
    ...
    EXPECT_NEAR(getPrimal(area_cut_face), getPrimal(area_selected_face - area_des_of_selected_face), 1e-5);

    // make sure that area_cut_face is not zero
    EXPECT_NEAR(getPrimal(area_selected_face - area_des_of_selected_face), 0.4275, 1e-3);
}