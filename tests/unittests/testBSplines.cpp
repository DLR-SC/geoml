...
TEST(BSplines, pointsToLinear)
{
    ...
    ASSERT_NEAR(0, getPrimal(curve->Value(0.  ).Distance(gp_Pnt(0,0,0))), 1e-10);
    ASSERT_NEAR(0, getPrimal(curve->Value(0.5 ).Distance(gp_Pnt(2,0,0))), 1e-10);
    ASSERT_NEAR(0, getPrimal(curve->Value(0.75).Distance(gp_Pnt(2,1,0))), 1e-10);
    ASSERT_NEAR(0, getPrimal(curve->Value(1.  ).Distance(gp_Pnt(1,1,0))), 1e-10);
    
    gp_Pnt p; gp_Vec v;
    curve->D1(0.25, p, v);
    ASSERT_NEAR(getPrimal(v.Magnitude()), getPrimal(v*gp_Vec(1,0,0)), 1e-10);
    
    curve->D1(0.6, p, v);
    ASSERT_NEAR(getPrimal(v.Magnitude()), getPrimal(v*gp_Vec(0,1,0)), 1e-10);
...
TEST(BSplines, bSplineMatDeriv0)
{
    ...
    EXPECT_NEAR(getPrimal(A.Value(1,1)), 1., 1e-10);
    EXPECT_NEAR(getPrimal(A.Value(1,2)), 0., 1e-10);
    EXPECT_NEAR(getPrimal(A.Value(1,3)), 0., 1e-10);

    EXPECT_NEAR(getPrimal(A.Value(2,1)), 4.444444444444445308e-01, 1e-10);
    EXPECT_NEAR(getPrimal(A.Value(2,2)), 4.444444444444444753e-01, 1e-10);
    EXPECT_NEAR(getPrimal(A.Value(2,3)), 1.111111111111111049e-01, 1e-10);

    EXPECT_NEAR(getPrimal(A.Value(3,1)), 1.111111111111111327e-01, 1e-10);
    EXPECT_NEAR(getPrimal(A.Value(3,2)), 4.444444444444444753e-01, 1e-10);
    EXPECT_NEAR(getPrimal(A.Value(3,3)), 4.444444444444444198e-01, 1e-10);

    EXPECT_NEAR(getPrimal(A.Value(4,1)), 0., 1e-10);
    EXPECT_NEAR(getPrimal(A.Value(4,2)), 0., 1e-10);
    EXPECT_NEAR(getPrimal(A.Value(4,3)), 1., 1e-10);
}
...
TEST(BSplines, bSplineMatDeriv1)
{
    ...
    // compare with python implementation
    EXPECT_NEAR(getPrimal(A.Value(1,1)), -2., 1e-10);
    EXPECT_NEAR(getPrimal(A.Value(1,2)), 2., 1e-10);
    EXPECT_NEAR(getPrimal(A.Value(1,3)), 0., 1e-10);

    EXPECT_NEAR(getPrimal(A.Value(2,1)), -1.333333333333333481e+00, 1e-10);
    EXPECT_NEAR(getPrimal(A.Value(2,2)), 6.666666666666668517e-01, 1e-10);
    EXPECT_NEAR(getPrimal(A.Value(2,3)), 6.666666666666666297e-01, 1e-10);

    EXPECT_NEAR(getPrimal(A.Value(3,1)), -6.666666666666667407e-01, 1e-10);
    EXPECT_NEAR(getPrimal(A.Value(3,2)), -6.666666666666665186e-01, 1e-10);
    EXPECT_NEAR(getPrimal(A.Value(3,3)), 1.333333333333333259e+00, 1e-10);

    EXPECT_NEAR(getPrimal(A.Value(4,1)), 0., 1e-10);
    EXPECT_NEAR(getPrimal(A.Value(4,2)), -2., 1e-10);
    EXPECT_NEAR(getPrimal(A.Value(4,3)), 2., 1e-10);
}
...
protected:
    ...
    void SetUp() override
    {
        ...
        while(!file.eof()) {
            ...
            std::vector<std::string> cols = geoml::split_string(line, ' ');
            if (cols.size() != 8) {
                continue;
            }
...
{
    geoml::BSplineApproxInterp app(pnts, 30, 3);
    ...
    EXPECT_NEAR(0.01317089, getPrimal(result.error), 1e-5);
    EXPECT_NEAR(0.0, getPrimal(result.curve->Value(parms[0]).Distance(pnts.Value(1))), 1e-10);
    EXPECT_NEAR(0.0, getPrimal(result.curve->Value(parms[50]).Distance(pnts.Value(51))), 1e-10);
    EXPECT_NEAR(0.0, getPrimal(result.curve->Value(parms[100]).Distance(pnts.Value(101))), 1e-10);

    result = app.FitCurveOptimal(parms);
    EXPECT_NEAR(0.000393704, getPrimal(result.error), 1e-5);
    EXPECT_NEAR(0.0, getPrimal(result.curve->Value(parms[0]).Distance(pnts.Value(1))), 1e-10);
    EXPECT_NEAR(0.0, getPrimal(result.curve->Value(parms[50]).Distance(pnts.Value(51))), 1e-10);
    EXPECT_NEAR(0.0, getPrimal(result.curve->Value(parms[100]).Distance(pnts.Value(101))), 1e-10);
...
TEST_F(BSplineInterpolation, approxAndInterpolateContinuous1)
{
    ...
    geoml::ApproxResult result = app.FitCurve();
...
TEST_F(BSplineInterpolation, interpolateAll)
{
    ...
    geoml::BSplineApproxInterp app(pnt2, nPoints, 2, false);
    for (int i = 0; i < nPoints; ++i) {
...
TEST_F(BSplineInterpolation, interpolateAllContinuous)
{
    ...
    geoml::BSplineApproxInterp app(pnt2, nPoints + 2, 2, true);
    for (int i = 0; i < nPoints; ++i) {
        app.InterpolatePoint(static_cast<size_t>(i));
    }
...
TEST_F(BSplineInterpolation, interpolateErrors)
{
    ...
    geoml::BSplineApproxInterp app(pnt2, nPoints, 2, true);
    for (int i = 0; i < nPoints; ++i) {
        app.InterpolatePoint(static_cast<size_t>(i));
    }

    // too few control points
    ASSERT_THROW(app.FitCurve(), geoml::Error);
    
    geoml::BSplineApproxInterp app2(pnt2, nPoints + 1, 2, false);
    for (int i = 0; i < nPoints; ++i) {
        app2.InterpolatePoint(static_cast<size_t>(i));
    }

    // too many control points
    ASSERT_THROW(app2.FitCurve(), geoml::Error);
}
...
TEST_F(BSplineInterpolation, approxOnly)
{
    ...
    geoml::ApproxResult result = app.FitCurve(parms);
    // Value from different splinelib implementation
    EXPECT_NEAR(0.01898, getPrimal(result.error), 1e-5);

    result = app.FitCurveOptimal(parms);
    EXPECT_NEAR(0.00238, getPrimal(result.error), 1e-5);
...
{
    geoml::BSplineApproxInterp app(pnts, 31, 3);
    app.InterpolatePoint(0);
    app.InterpolatePoint(100);
    geoml::ApproxResult result = app.FitCurve(parms);
    EXPECT_NEAR(0.0, getPrimal(result.curve->Value(parms[0]).Distance(pnts.Value(1))), 1e-10);
    EXPECT_NEAR(0.0, getPrimal(result.curve->Value(parms[100]).Distance(pnts.Value(101))), 1e-10);
    EXPECT_NEAR(0.0055298, getPrimal(result.error), 1e-5);
...
TEST_F(BSplineInterpolation, withKinksShield)
{
    ...
    geoml::ParamMap params = {
        {3, 0.8},
        {1, 0.2},
    };

    std::vector<unsigned int> kinks = {1, 3};
    geoml::InterpolatePointsWithKinks interpolator(points, kinks, params, 0.5);
    ...
    auto kinkParams = geoml::BSplineAlgorithms::getKinkParameters(curve);
...
TEST_F(BSplineInterpolation, withKinksSlit)
{
    ...
    auto paramsVec = interpolator.Parameters();
    EXPECT_NEAR(0.0, getPrimal(paramsVec[0]), getPrimal(tol));
    EXPECT_NEAR(0.5, getPrimal(paramsVec[2]), getPrimal(tol));
    EXPECT_NEAR(1.0, getPrimal(paramsVec[4]), getPrimal(tol));
...
TEST_F(BSplineInterpolation, interpolationContinuous)
{
    ...
    for (int degree = 1; degree <= 4; ++degree) {
        ...
        geoml::PointsToBSplineInterpolation app(pnt2, degree, true);
        ...
        for (int iparm = 0; iparm < params.size(); ++iparm) {
            ...
            EXPECT_NEAR(0., getPrimal(p.Distance(pnt2->Value(iparm + 1))), 1e-10);
        }
...
TEST_F(BSplineInterpolation, interpolationClosedIssue1)
{
    ...
    geoml::PointsToBSplineInterpolation app(pnt2, params, degree, true);
    ...
    EXPECT_NEAR(0., getPrimal((curve->DN(0., 1) - curve->DN(1., 1)).Magnitude()), 1e-10);
    EXPECT_NEAR(0., getPrimal((curve->DN(0., 2) - curve->DN(1., 2)).Magnitude()), 1e-10);
...
TEST_F(BSplineInterpolation, errorParamsNotResized)
{
    ...
    EXPECT_NEAR(0., getPrimal(fit.GetMaxError()), 2e-3);
