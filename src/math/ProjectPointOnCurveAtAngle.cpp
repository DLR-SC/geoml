...
namespace 
{
                                     ...
                                     const gp_Dir& m_refNormal, double m_angle)
    {
        ...
        geoml::normalize(planeNormal, planeNormal_deriv);

        geoml::ValueWithDerivative distance = geoml::distancePlanePoint_deriv(planeNormal, planeNormal_deriv,
                                                                            c, c1,
                                                                            m_pointToProject);

        // compute square distance to improve convergence
        geoml::ValueWithDerivative result;
...
bool geoml::ProjectPointOnCurveAtAngle::IsDone() const
{
    Compute();
...
gp_Pnt geoml::ProjectPointOnCurveAtAngle::Point(int i) const
{
    if (!IsDone()) {
...
int geoml::ProjectPointOnCurveAtAngle::NbPoints() const
{
    Compute();
...
void geoml::ProjectPointOnCurveAtAngle::Compute() const
{
    if (m_hasComputed) {
        return;
    }
...
void geoml::ProjectPointOnCurveAtAngle::AddSolution(double value, double tol) const
{
    ...
    geoml::ValueWithDerivative result = objFun(m_pointToProject, value, m_curve, m_refNormal, m_angle);
