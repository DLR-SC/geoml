...
public:
     ...
     */
    GEOML_EXPORT CurveNetworkSorter(const std::vector<Handle(Geom_Curve)>& profiles,
     ...
     */
    GEOML_EXPORT void GetStartCurveIndices(size_t& prof_idx, size_t& guid_idx, bool& guideMustBeReversed) const;
     ...
     */
    GEOML_EXPORT size_t NProfiles() const;

    /**
     * @brief Returns the number of guide curves of the network
     */
    GEOML_EXPORT size_t NGuides() const;
    ...
    GEOML_EXPORT std::vector<std::string> const & ProfileIndices() const;
    GEOML_EXPORT std::vector<std::string> const & GuideIndices() const;
