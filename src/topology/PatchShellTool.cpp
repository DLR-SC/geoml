...
void PatchShellTool::AddSideCap(TopoDS_Wire const& boundaryWire)
{
...
TopoDS_Shape PatchShellTool::PatchedShape()
{
    if( !_hasPerformed ) {
        Perform();
    }
...
void PatchShellTool::Perform()
{
    ...
    if (_makeSolid) {
        ...
        try {
            ...
            throw Error("Cannot make a solid out of the shell. Is the base type correct?", geoml::GENERIC_ERROR);
        }
...
{
    if (shell.IsNull()) {
        throw geoml::Error("Loft is not build", geoml::GENERIC_ERROR);
    }
    ...
    try {
        ...
        if ( shellClosed.ShapeType() != TopAbs_SHELL ) {
            ...
            if ( shellClosed.ShapeType() != TopAbs_FACE) {
                throw geoml::Error("Cannot patch a shape that is neither a shell nor a face");
            }
        ...
        throw geoml::Error("Error in geometry creation: Could not create a closed shell.", geoml::GENERIC_ERROR);
    }
}
