...
public:
     ...
     */
    GEOML_EXPORT void SetMaxDegree(int degree);
...
private:
    GEOML_EXPORT void CalculateParameters(std::vector<Handle(Geom_BSplineCurve)> const& splines_vector);
    GEOML_EXPORT void Perform();
    GEOML_EXPORT void Invalidate() { _hasPerformed = false; }
