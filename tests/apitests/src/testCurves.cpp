...
EXPECT_NEAR(
    getPrimal(curve->StartPoint().X()), 
    getPrimal(curve->EndPoint().X()), 
    1e-5);

EXPECT_NEAR(
    getPrimal(curve->StartPoint().Y()), 
    getPrimal(curve->EndPoint().Y()), 
    1e-5);

EXPECT_NEAR(
    getPrimal(curve->StartPoint().Z()), 
    getPrimal(curve->EndPoint().Z()), 
    1e-5);
...
EXPECT_NEAR(getPrimal(length), 12.566, 1e-3);
...
TEST(Test_interpolate_points_to_b_spline_curve, simple_interpolation_of_points)
{
    ...
    EXPECT_NEAR(getPrimal(curve->StartPoint().X()), getPrimal(pt_1.X()), 1e-5);
    EXPECT_NEAR(getPrimal(curve->StartPoint().Y()), getPrimal(pt_1.Y()), 1e-5);
    EXPECT_NEAR(getPrimal(curve->StartPoint().Z()), getPrimal(pt_1.Z()), 1e-5);

    EXPECT_NEAR(getPrimal(curve->EndPoint().X()), getPrimal(pt_3.X()), 1e-5);
    EXPECT_NEAR(getPrimal(curve->EndPoint().Y()), getPrimal(pt_3.Y()), 1e-5);
    EXPECT_NEAR(getPrimal(curve->EndPoint().Z()), getPrimal(pt_3.Z()), 1e-5);
...
TEST(Test_interpolate_points_to_b_spline_curve, interpolation_custom_parameters)
{
    ...
    EXPECT_NEAR(getPrimal(test_point.X()), 1.0, 1e-5);
    EXPECT_NEAR(getPrimal(test_point.Y()), 0.0, 1e-5);
    EXPECT_NEAR(getPrimal(test_point.Z()), 0.0, 1e-5);
...
TEST_F(CurvesToBlend, blend_curve_test_1)
{
    ...
    EXPECT_NEAR(getPrimal(bezier_curve->StartPoint().X()), 2., 1e-5);
    EXPECT_NEAR(getPrimal(bezier_curve->StartPoint().Y()), 2., 1e-5);
    EXPECT_NEAR(getPrimal(bezier_curve->StartPoint().Z()), 0., 1e-5);

    EXPECT_NEAR(getPrimal(bezier_curve->EndPoint().X()), 4., 1e-5);
    EXPECT_NEAR(getPrimal(bezier_curve->EndPoint().Y()), 4., 1e-5);
    EXPECT_NEAR(getPrimal(bezier_curve->EndPoint().Z()), 0., 1e-5);
}
...
TEST_F(CurvesToBlend, blend_curve_test_2)
{
    ...
    EXPECT_NEAR(getPrimal(bezier_curve->Pole(2).X()), getPrimal(last_point_start_curve.X() + first_derivative_start_curve_at_last_parameter.X()), 1e-5);
    EXPECT_NEAR(getPrimal(bezier_curve->Pole(2).Y()), getPrimal(last_point_start_curve.Y() + first_derivative_start_curve_at_last_parameter.Y()), 1e-5);
    EXPECT_NEAR(getPrimal(bezier_curve->Pole(2).Z()), getPrimal(last_point_start_curve.Z() + first_derivative_start_curve_at_last_parameter.Z()), 1e-5);
}
...
TEST_F(CurvesToBlend, blend_curve_test_3)
{
    ...
    EXPECT_NEAR(getPrimal(bezier_curve->Pole(2).X()), getPrimal(first_point_end_curve.X() - first_derivative_end_curve_at_first_parameter.X()), 1e-5);
    EXPECT_NEAR(getPrimal(bezier_curve->Pole(2).Y()), getPrimal(first_point_end_curve.Y() - first_derivative_end_curve_at_first_parameter.Y()), 1e-5);
    EXPECT_NEAR(getPrimal(bezier_curve->Pole(2).Z()), getPrimal(first_point_end_curve.Z() - first_derivative_end_curve_at_first_parameter.Z()), 1e-5);
}
...
TEST_F(CurvesToBlend, blend_curve_test_4)
{
    ...
    EXPECT_NEAR(getPrimal(bezier_curve->Pole(3).X()), getPrimal(last_point_start_curve.X() + 2 * first_derivative_start_curve_at_last_parameter.X() + d_vec_curve_start.X()), 1e-5);
    EXPECT_NEAR(getPrimal(bezier_curve->Pole(3).Y()), getPrimal(last_point_start_curve.Y() + 2 * first_derivative_start_curve_at_last_parameter.Y() + d_vec_curve_start.Y()), 1e-5);
    EXPECT_NEAR(getPrimal(bezier_curve->Pole(3).Z()), getPrimal(last_point_start_curve.Z() + 2 * first_derivative_start_curve_at_last_parameter.Z() + d_vec_curve_start.Z()), 1e-5);
}
...
TEST_F(CurvesToBlend, blend_curve_test_5)
{
    ...
    EXPECT_NEAR(getPrimal(bezier_curve->Pole(2).X()), getPrimal(first_point_end_curve.X() - 2 * first_derivative_end_curve_at_first_parameter.X() + d_vec_curve_end.X()), 1e-5);
    EXPECT_NEAR(getPrimal(bezier_curve->Pole(2).Y()), getPrimal(first_point_end_curve.Y() - 2 * first_derivative_end_curve_at_first_parameter.Y() + d_vec_curve_end.Y()), 1e-5);
    EXPECT_NEAR(getPrimal(bezier_curve->Pole(2).Z()), getPrimal(first_point_end_curve.Z() - 2 * first_derivative_end_curve_at_first_parameter.Z() + d_vec_curve_end.Z()), 1e-5);

    EXPECT_NEAR(getPrimal(bezier_curve->Pole(3).X()), getPrimal(first_point_end_curve.X() - first_derivative_end_curve_at_first_parameter.X()), 1e-5);
    EXPECT_NEAR(getPrimal(bezier_curve->Pole(3).Y()), getPrimal(first_point_end_curve.Y() - first_derivative_end_curve_at_first_parameter.Y()), 1e-5);
    EXPECT_NEAR(getPrimal(bezier_curve->Pole(3).Z()), getPrimal(first_point_end_curve.Z() - first_derivative_end_curve_at_first_parameter.Z()), 1e-5);
}
...
TEST_F(CurvesToBlend, blend_curve_test_6)
{
    ...
    EXPECT_NEAR(getPrimal(bezier_curve->Pole(4).X()), getPrimal(first_point_end_curve.X() - first_derivative_end_curve_at_first_parameter.X()), 1e-5);
    EXPECT_NEAR(getPrimal(bezier_curve->Pole(4).Y()), getPrimal(first_point_end_curve.Y() - first_derivative_end_curve_at_first_parameter.Y()), 1e-5);
    EXPECT_NEAR(getPrimal(bezier_curve->Pole(4).Z()), getPrimal(first_point_end_curve.Z() - first_derivative_end_curve_at_first_parameter.Z()), 1e-5);
}
...
TEST_F(CurvesToBlend, blend_curve_test_7)
{
    ...
    EXPECT_NEAR(getPrimal(bezier_curve->Pole(3).X()), getPrimal(first_point_end_curve.X() - 2 * first_derivative_end_curve_at_first_parameter.X() + d_vec_curve_end.X()), 1e-5);
    EXPECT_NEAR(getPrimal(bezier_curve->Pole(3).Y()), getPrimal(first_point_end_curve.Y() - 2 * first_derivative_end_curve_at_first_parameter.Y() + d_vec_curve_end.Y()), 1e-5);
    EXPECT_NEAR(getPrimal(bezier_curve->Pole(3).Z()), getPrimal(first_point_end_curve.Z() - 2 * first_derivative_end_curve_at_first_parameter.Z() + d_vec_curve_end.Z()), 1e-5);
...
TEST_F(CurvesToBlend, blend_curve_test_8)
{
    ...
    EXPECT_NEAR(getPrimal(bezier_curve->Pole(4).X()), getPrimal(first_point_end_curve.X() - 2 * first_derivative_end_curve_at_first_parameter.X() + d_vec_curve_end.X()), 1e-5);
    EXPECT_NEAR(getPrimal(bezier_curve->Pole(4).Y()), getPrimal(first_point_end_curve.Y() - 2 * first_derivative_end_curve_at_first_parameter.Y() + d_vec_curve_end.Y()), 1e-5);
    EXPECT_NEAR(getPrimal(bezier_curve->Pole(4).Z()), getPrimal(first_point_end_curve.Z() - 2 * first_derivative_end_curve_at_first_parameter.Z() + d_vec_curve_end.Z()), 1e-5);

    EXPECT_NEAR(getPrimal(bezier_curve->Pole(5).X()), getPrimal(first_point_end_curve.X() - first_derivative_end_curve_at_first_parameter.X()), 1e-5);
    EXPECT_NEAR(getPrimal(bezier_curve->Pole(5).Y()), getPrimal(first_point_end_curve.Y() - first_derivative_end_curve_at_first_parameter.Y()), 1e-5);
    EXPECT_NEAR(getPrimal(bezier_curve->Pole(5).Z()), getPrimal(first_point_end_curve.Z() - first_derivative_end_curve_at_first_parameter.Z()), 1e-5);
}
...
TEST_F(CurvesToBlend, blend_curve_test_14)
{
    ...
    EXPECT_NEAR(getPrimal(bezier_curve->Pole(2).X()), getPrimal(last_point_start_curve.X() + 0.5 * first_derivative_start_curve_at_last_parameter.X()), 1e-5);
    EXPECT_NEAR(getPrimal(bezier_curve->Pole(2).Y()), getPrimal(last_point_start_curve.Y() + 0.5 * first_derivative_start_curve_at_last_parameter.Y()), 1e-5);
    EXPECT_NEAR(getPrimal(bezier_curve->Pole(2).Z()), getPrimal(last_point_start_curve.Z() + 0.5 * first_derivative_start_curve_at_last_parameter.Z()), 1e-5);
}
...
TEST_F(CurvesToBlend, blend_curve_test_15)
{
    ...
    EXPECT_NEAR(getPrimal(bezier_curve->Pole(2).X()), getPrimal(first_point_end_curve.X() - 0.5 * first_derivative_end_curve_at_first_parameter.X()), 1e-5);
    EXPECT_NEAR(getPrimal(bezier_curve->Pole(2).Y()), getPrimal(first_point_end_curve.Y() - 0.5 * first_derivative_end_curve_at_first_parameter.Y()), 1e-5);
    EXPECT_NEAR(getPrimal(bezier_curve->Pole(2).Z()), getPrimal(first_point_end_curve.Z() - 0.5 * first_derivative_end_curve_at_first_parameter.Z()), 1e-5);
}
...
TEST_F(CurvesToBlend, blend_curve_test_16)
{
    ...
    EXPECT_NEAR(getPrimal(bezier_curve->Pole(3).X()), getPrimal(last_point_start_curve.X() + 2 * 0.5 * 2 * first_derivative_start_curve_at_last_parameter.X() + d_vec_curve_end.X()), 1e-5);
    EXPECT_NEAR(getPrimal(bezier_curve->Pole(3).Y()), getPrimal(last_point_start_curve.Y() + 2 * 0.5 * 2 * first_derivative_start_curve_at_last_parameter.Y() + d_vec_curve_end.Y()), 1e-5);
    EXPECT_NEAR(getPrimal(bezier_curve->Pole(3).Z()), getPrimal(last_point_start_curve.Z() + 2 * 0.5 * 2 * first_derivative_start_curve_at_last_parameter.Z() + d_vec_curve_end.Z()), 1e-5);
}
...
TEST_F(CurvesToBlend, blend_curve_test_17)
{
    ...
    EXPECT_NEAR(getPrimal(bezier_curve->Pole(2).X()), getPrimal(first_point_end_curve.X() - 2 * 0.5 * 2 * first_derivative_end_curve_at_first_parameter.X() + d_vec_curve_end.X()), 1e-5);
    EXPECT_NEAR(getPrimal(bezier_curve->Pole(2).Y()), getPrimal(first_point_end_curve.Y() - 2 * 0.5 * 2 * first_derivative_end_curve_at_first_parameter.Y() + d_vec_curve_end.Y()), 1e-5);
    EXPECT_NEAR(getPrimal(bezier_curve->Pole(2).Z()), getPrimal(first_point_end_curve.Z() - 2 * 0.5 * 2 * first_derivative_end_curve_at_first_parameter.Z() + d_vec_curve_end.Z()), 1e-5);

    EXPECT_NEAR(getPrimal(bezier_curve->Pole(3).X()), getPrimal(first_point_end_curve.X() - 0.5 * first_derivative_end_curve_at_first_parameter.X()), 1e-5);
    EXPECT_NEAR(getPrimal(bezier_curve->Pole(3).Y()), getPrimal(first_point_end_curve.Y() - 0.5 * first_derivative_end_curve_at_first_parameter.Y()), 1e-5);
    EXPECT_NEAR(getPrimal(bezier_curve->Pole(3).Z()), getPrimal(first_point_end_curve.Z() - 0.5 * first_derivative_end_curve_at_first_parameter.Z()), 1e-5);
}
...
TEST_F(CurvesToBlend, blend_curve_test_18)
{
    ...
    EXPECT_NEAR(getPrimal(bezier_curve->Pole(2).X()), getPrimal(last_point_start_curve.X() + 0.25 * first_derivative_start_curve_at_last_parameter.X()), 1e-5);
    EXPECT_NEAR(getPrimal(bezier_curve->Pole(2).Y()), getPrimal(last_point_start_curve.Y() + 0.25 * first_derivative_start_curve_at_last_parameter.Y()), 1e-5);
    EXPECT_NEAR(getPrimal(bezier_curve->Pole(2).Z()), getPrimal(last_point_start_curve.Z() + 0.25 * first_derivative_start_curve_at_last_parameter.Z()), 1e-5);
    ...
    EXPECT_NEAR(getPrimal(bezier_curve->Pole(3).X()), getPrimal(last_point_start_curve.X() + 0.25 * 2 * first_derivative_start_curve_at_last_parameter.X() + d_vec_curve_start.X()), 1e-5);
    EXPECT_NEAR(getPrimal(bezier_curve->Pole(3).Y()), getPrimal(last_point_start_curve.Y() + 0.25 * 2 * first_derivative_start_curve_at_last_parameter.Y() + d_vec_curve_start.Y()), 1e-5);
    EXPECT_NEAR(getPrimal(bezier_curve->Pole(3).Z()), getPrimal(last_point_start_curve.Z() + 0.25 * 2 * first_derivative_start_curve_at_last_parameter.Z() + d_vec_curve_start.Z()), 1e-5);
    ...
    EXPECT_NEAR(getPrimal(bezier_curve->Pole(4).X()), getPrimal(first_point_end_curve.X() - 2 * 2 * first_derivative_end_curve_at_first_parameter.X() + d_vec_curve_end.X()), 1e-5);
    EXPECT_NEAR(getPrimal(bezier_curve->Pole(4).Y()), getPrimal(first_point_end_curve.Y() - 2 * 2 * first_derivative_end_curve_at_first_parameter.Y() + d_vec_curve_end.Y()), 1e-5);
    EXPECT_NEAR(getPrimal(bezier_curve->Pole(4).Z()), getPrimal(first_point_end_curve.Z() - 2 * 2 * first_derivative_end_curve_at_first_parameter.Z() + d_vec_curve_end.Z()), 1e-5);
