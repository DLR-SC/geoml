...
#ifndef RECTGRIDSURFACE_H
#define RECTGRIDSURFACE_H
...
/**
 * @brief The RectGridSurface class represents
...
public:
    ...
    GEOML_EXPORT RectGridSurface(){}

    /**
     * @brief RectGridSurface constructs the surface
     * from a shape
     ...
     */
    template <typename... Args>
    RectGridSurface(TopoDS_Shape const& shape, Args&&... args)
    {
    ...
    // RectGridSurface is move-only, because internally some pointers are stored.
    RectGridSurface(RectGridSurface const&) = delete;
    RectGridSurface& operator=(RectGridSurface const&) = delete;
    RectGridSurface(RectGridSurface&&) = default;
    RectGridSurface& operator=(RectGridSurface&&) = default;
     ...
     *
     * The vector row_ranges will contain the sum of all u-parameter ranges of the faces
     * in the row corresponding to the index in the vector
     *
     * The vector col_ranges will contain the sum of all v-parameter ranges of the faces
    ...
    void connect_faces()
    {
        ...
        for (auto& face : face_infos ) {
            if (!face.UPrev()){
                ...
                {}
                if (ncols_tmp != ncols ) {
                    throw Error("The Input Shape is not a rectangular grid of faces");
                }
            }
            if (!face.VPrev()) {
