...
void BSplineApproxInterp::InterpolatePoint(size_t pointIndex, bool withKink)
{
    ...
    if (it == m_indexOfApproximated.end()) {
        throw Error("Invalid index in BSplineApproxInterp::InterpolatePoint", geoml::INDEX_ERROR);
    }
...
ApproxResult BSplineApproxInterp::solve(const std::vector<Standard_Real>& params, const TColStd_Array1OfReal& knots, const TColStd_Array1OfInteger& mults) const
{
    ...
    if (nCtrPnts < n_intpolated + n_continuityConditions || nCtrPnts < m_degree + 1 + n_continuityConditions) {
        throw geoml::Error("Too few control points for curve interpolation!");
    }

    if (n_apprxmated == 0 && nCtrPnts != n_intpolated + n_continuityConditions) {
        throw geoml::Error("Wrong number of control points for curve interpolation!");
    }
    ...
    if (!solver.IsDone()) {
        throw Error("Singular Matrix", geoml::MATH_ERROR);
    }
