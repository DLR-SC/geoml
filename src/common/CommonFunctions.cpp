    ...
    Standard_Real len =  GCPnts_AbscissaPoint::Length( aCompoundCurve );
     if (len < Precision::Confusion()) {
         ...
         if (algo.IsDone()) {
             ...
             throw geoml::Error("WireGetPointTangent: Cannot compute point on curve.", geoml::MATH_ERROR);
          }
     }
...
void EdgeGetPointTangent(const TopoDS_Edge& edge, double alpha, gp_Pnt& point, gp_Vec& tangent)
{
    ...
    if (len < Precision::Confusion()) {
          throw geoml::Error("EdgeGetPointTangent: Unable to compute tangent on zero length edge", geoml::MATH_ERROR);
    }
    ...
    if (algo.IsDone()) {
        ...
        throw geoml::Error("EdgeGetPointTangent: Cannot compute point on curve.", geoml::MATH_ERROR);
    }
}
...
Standard_Real ProjectPointOnWireAtAngle(const TopoDS_Wire &wire, gp_Pnt p, gp_Dir rotationAxisAroundP, double angle)
{
    ...
    for (wireExplorer.Init(wire); wireExplorer.More(); wireExplorer.Next(), iwire++) {
        ...
        if (fabs(angle - M_PI / 2.) < 1e-6) {
            ...
            geoml::ProjectPointOnCurveAtAngle proj(p, trimmedCurve, angle, rotationAxisAroundP);
...
gp_Pnt ProjectPointOnShape(const TopoDS_Shape &shape, const gp_Pnt &point, const gp_Vec &direction)
{
    ...
    if (nPoints == 0) {
        throw geoml::Error("Projection of point to shape failed.", geoml::MATH_ERROR);
    }
    else if (nPoints > 1) {
                     ...
                     double vmax)
{
    ...
    Handle(Geom_BSplineSurface) trimmed_surf
        = geoml::BSplineAlgorithms::trimSurface(surf, umin, umax, vmin, vmax);
...
ListPNamedShape GroupFaces(const PNamedShape shape, geoml::ShapeGroupMode groupType)
{
    ...
    if (groupType == geoml::NAMED_COMPOUNDS) {
        BRep_Builder b;
        TopTools_IndexedMapOfShape faceMap;
    ...
    else if (groupType == geoml::WHOLE_SHAPE) {
        shapeList.push_back(shape);
    }
    else if (groupType == geoml::FACES) {
...
GEOML_EXPORT bool GetIntersectionPoint(const TopoDS_Wire& wire1, const TopoDS_Wire& wire2, intersectionPointList& intersectionPoints, const double tolerance)
{
        ...
        while ( EdgeExplorer1.More() ) {
            ...
            if (!Intersector.IsDone()) {
                LOG(ERROR) << "Error intersecting two wires in GetIntersectionPoint!";
                    throw geoml::Error("Error intersecting two wires in GetIntersectionPoint!");
            }
    ...
    if ( intersectionPoints.size() == 0 ) {
        LOG(INFO) << "GetIntersectionPoint: The curves do not intersect to the specified tolerance";
        return false;
    }
...
TopoDS_Face GetSingleFace(const TopoDS_Shape& shape)
{
    ...
    if (numFaces < 1) {
        ...
        throw geoml::Error("unable to get single face from shape: shape contains no faces!");
    }
    else if (numFaces > 1) {
        LOG(ERROR) << "unable to get single face from shape: shape contains more than one face";
        throw geoml::Error("unable to get single face from shape: shape contains more than one face!");
    }
...
gp_Pnt GetFirstPoint(const TopoDS_Shape& wireOrEdge)
{
    ...
    throw geoml::Error("Shape must be wire or edge");
}
...
geoml::Continuity getEdgeContinuity(const TopoDS_Edge& edge1, const TopoDS_Edge& edge2)
{
    // **********************************************************************************
    ...
    wireMaker.Add(edge1);
    if (!wireMaker.IsDone()) {
        throw geoml::Error("checkEdgeContinuity: Unable to create connected wire", geoml::GENERIC_ERROR);
    }
    wireMaker.Add(edge2);
    if (!wireMaker.IsDone()) {
        throw geoml::Error("checkEdgeContinuity: Unable to create connected wire", geoml::GENERIC_ERROR);
    ...
    wire = wireFixer.Wire();
    if (wire.IsNull()) {
        throw geoml::Error("checkEdgeContinuity: Unable to create common vertex in connected wire", geoml::GENERIC_ERROR);
    }
    ...
    if (edges.Extent() > 2) {
        throw geoml::Error("checkEdgeContinuity: Unexpected error in connected wire", geoml::GENERIC_ERROR);
    }
    ...
    if (commonVertex.IsNull()) {
        throw geoml::Error("checkEdgeContinuity: Unable to find common vertex", geoml::GENERIC_ERROR);
    }
    ...
    if (TopExp::FirstVertex(newedge1).IsSame(commonVertex) && TopExp::LastVertex(newedge2).IsSame(commonVertex)) {
        ...
        throw geoml::Error("checkEdgeContinuity: Unexpected error: Unable to find common vertex in connected wire", geoml::GENERIC_ERROR);
    }
...
TopoDS_Face BuildFace(const TopoDS_Wire& wire)
{
    ...
    throw geoml::Error("BuildFace: Unable to generate face from Wire!");
}
...
TopoDS_Face BuildRuledFace(const TopoDS_Wire& wire1, const TopoDS_Wire& wire2)
{
    ...
    if (!orderWires.IsDone()) {
        ...
        throw geoml::Error("unable to determine common wire order!");
    }
    ...
    if (orderedWireSequence.Length() != 2) {
        ...
        throw geoml::Error("number of wires in sequence does not match. Expected: 2");
    }
    ...
    if (!approx1.IsDone() || !approx1.HasResult() ||
        ...
        throw geoml::Error("unable to approximate wires by curves for building face!");
    }
...
TopoDS_Wire BuildWireFromEdges(const TopoDS_Shape& edges)
{
    ...
    if (wireList.Extent() != 1) {
        ...
        throw geoml::Error("Error generating single wire in MakeWireFromEdges!");
    }
...
void BuildWiresFromConnectedEdges(const TopoDS_Shape& shape, TopTools_ListOfShape& wireList)
{
    ...
    while (edgeList.Extent() > 0) {
        ...
        if (!wireBuilder.IsDone()) {
            ...
            throw geoml::Error("Error creating wire in BuildWiresFromConnectedEdges!");
        }
...
TopoDS_Wire CloseWire(const TopoDS_Wire& wire)
{
    ...
    if (numEndVertices != 2) {
        ...
        throw geoml::Error("invalid number of end vertices found in CloseWire!");
    }
    ...
    if (!makeWire.IsDone()) {
        ...
        throw geoml::Error("unable to build closed wire in CloseWire!");
    }
...
TopoDS_Wire CloseWires(const TopoDS_Wire& wire1, const TopoDS_Wire& wire2)
{
    ...
    if (endVertices1.Extent() != 2) {
        ...
        throw geoml::Error("Unable to close wires because invalid number of end-vertices found!");
    }
...
TopoDS_Wire CloseWires(const TopoDS_Wire& wire1, const TopoDS_Wire& wire2, const gp_Vec& dir)
{
    ...
    if (!makeWire.IsDone()) {
        ...
        throw geoml::Error("error during creation of closed wire in CloseWires!");
    }
...
TopoDS_Shape CutShapes(const TopoDS_Shape& shape1, const TopoDS_Shape& shape2)
{
    ...
    if (!cutter.IsDone()) {
        ...
        throw geoml::Error("Error cutting shapes in method CutShapes!");
    }
    return cutter.Shape();
}
...
TopoDS_Face GetNearestFace(const TopoDS_Shape& shape, const gp_Pnt& pnt)
{
    ...
    for (explorer.Init(shape, TopAbs_FACE); explorer.More(); explorer.Next()) {
        ...
        if (!extrema.IsDone() || extrema.NbSolution() < 1) {
            ...
            throw geoml::Error("unable to determine nearest point between face and vertex!");
        }
    ...
    for (std::vector<double>::const_iterator it = breaks.begin(); it != breaks.end(); ++it) {
        ...
        if (pos != result.end()) {
            ...
            *pos = breakpoint;
            ...
            pos = std::find_if(result.begin(), result.end(), geoml::IsInsideTolerance(breakpoint, (0.5 + 1e-8)*du));
...
TopoDS_Shape GetFacesByName(const PNamedShape shape, const std::string &name)
{
    ...
    if (faces.empty())
        throw geoml::Error("Could not find faces named " + name);
    if (faces.size() == 1)
        return faces[0];
...
Handle(TColgp_HArray1OfPnt) OccArray(const std::vector<geoml::Point>& pnts)
{
    Handle(TColgp_HArray1OfPnt) result = new TColgp_HArray1OfPnt(1, static_cast<int>(pnts.size()));
    int idx = 1;
    for (std::vector<geoml::Point>::const_iterator it = pnts.begin(); it != pnts.end(); ++it, ++idx) {
