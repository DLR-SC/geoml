...
#include "InterpolatePointsWithKinks.h"
...
namespace
{
    ...
    template <>
    inline bool in(unsigned int key, const geoml::ParamMap& map)
    {
...
InterpolatePointsWithKinks::InterpolatePointsWithKinks(const Handle(TColgp_HArray1OfPnt) & points,
    ...
    , m_result(*this, &InterpolatePointsWithKinks::ComputeResult)
{
}

Handle(Geom_BSplineCurve) InterpolatePointsWithKinks::Curve() const
{
...
void InterpolatePointsWithKinks::ComputeResult(Result& result) const
{
    ...
    for (size_t i=1; i < kinks.size(); ++i) {
        ...
        auto segment_curve = PointsToBSplineInterpolation(current_pnts, the_pars, m_maxDegree, false).Curve();
    ...
    if (make_continuous && kinks.size() > 0) {
        ...
        BSplineAlgorithms::reparametrizeBSpline(*last, the_pars.front() - offset, param_to_split - offset);

        curve_segments.insert(curve_segments.begin(), first);
        curve_segments.push_back(last);
    }
    else if (make_continuous && kinks.empty()) {
        auto curve = PointsToBSplineInterpolation(m_pnts, new_params, m_maxDegree, true).Curve();
        curve_segments.push_back(curve);
    }
    
    result.curve = BSplineAlgorithms::concatCurves(curve_segments, false);
