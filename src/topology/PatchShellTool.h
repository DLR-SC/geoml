...
/**
 * @brief The PatchShellTool class can be used to create watertight geometries
 ...
 * a closed shell.
 */
class PatchShellTool
{
public:
    /**
     * @brief PatchShellTool constructor of the shell patcher. Input is a TopoDS_Shell,
     ...
     */
    GEOML_EXPORT void SetMakeSolid(bool enabled = true) { _makeSolid = enabled; }
     ...
     */
    GEOML_EXPORT void AddSideCap(TopoDS_Wire const&  boundaryWire);

    /**
     * @brief PatchedShape returns the patched shape calculated by the algorithm
     * @return the patched shape
     */
    GEOML_EXPORT TopoDS_Shape PatchedShape();
