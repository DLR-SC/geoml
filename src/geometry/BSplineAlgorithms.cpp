...
namespace
{
    ...
    {
    public:
        helper_function_unique(Standard_Real tolerance = 1e-15)
            : _tol(tolerance)
        {}

        // helper function for std::unique
        bool operator()(Standard_Real a, Standard_Real b)
        {
    ...
    public:
        ...
        void insertKnot(Standard_Real knot, int mult, Standard_Real tolerance=1e-15)
        {
            if (_dir == udir) {
        ...
        Standard_Real getKnot(int idx) const
        {
            if (_dir == udir) {
    ...
    bool haveSameRange(const std::vector<SplineAdapter>& splines_vector, Standard_Real par_tolerance)
    {
        Standard_Real begin_param_dir = splines_vector[0].getKnot(1);
        Standard_Real end_param_dir = splines_vector[0].getKnot(splines_vector[0].getNKnots());
     ...
     */
    template <class SplineAdapter>
    void makeGeometryCompatibleImpl(std::vector<SplineAdapter>& splines_vector, Standard_Real par_tolerance)
    {
        ...
        for (typename std::vector<SplineAdapter>::const_iterator splineIt = splines_vector.begin()+1; splineIt != splines_vector.end(); ++splineIt) {
            ...
            for (int knot_idx = 2; knot_idx < spline.getNKnots(); ++knot_idx) {
                Standard_Real knot = spline.getKnot(knot_idx);
        ...
        for (typename std::vector<SplineAdapter>::iterator splineIt = splines_vector.begin()+1; splineIt != splines_vector.end(); ++splineIt) {
            ...
            for (int knot_idx = 2; knot_idx < firstSpline.getNKnots(); ++knot_idx) {
                Standard_Real knot = firstSpline.getKnot(knot_idx);
...
const Standard_Real BSplineAlgorithms::REL_TOL_CLOSED = 1e-8;

bool BSplineAlgorithms::isUDirClosed(const TColgp_Array2OfPnt& points, Standard_Real tolerance)
{
...
bool BSplineAlgorithms::isVDirClosed(const TColgp_Array2OfPnt& points, Standard_Real tolerance)
{
...
std::vector<Standard_Real> BSplineAlgorithms::knotsFromCurveParameters(std::vector<Standard_Real> &params, unsigned int degree, bool closedCurve)
{
    if (params.size() < 2) {
    ...
    std::vector<Standard_Real> innerKnots(nInnerKnots);
    ...
    if (closedCurve && degree % 2 == 0) {
        ...
        std::vector<Standard_Real> dparm(m + 1, 0.);
        ...
        for (size_t j = 1; j < params.size() - degree; ++j) {
            ...
            innerKnots[j] = sum / static_cast<Standard_Real>(degree);
        }
    }

    if (closedCurve) {
        Standard_Real offset = innerKnots[0] - innerKnots[nInnerKnots -1];
...
Standard_Real BSplineAlgorithms::scale(const TColgp_Array2OfPnt& points)
{
    Standard_Real theScale = 0.;
    for (int uidx = points.LowerRow(); uidx <= points.UpperRow(); ++uidx) {
        ...
        for (int vidx = points.LowerCol() + 1; vidx <= points.UpperCol(); ++vidx) {
            Standard_Real dist = pFirst.Distance(points.Value(uidx, vidx));
...
Standard_Real BSplineAlgorithms::scale(const TColgp_Array1OfPnt& points)
{
    Standard_Real theScale = 0.;
...
std::vector<Standard_Real> BSplineAlgorithms::computeParamsBSplineCurve(const Handle(TColgp_HArray1OfPnt)& points, const Standard_Real alpha)
{
    return computeParamsBSplineCurve(points, 0., 1., alpha);
}

std::vector<Standard_Real> BSplineAlgorithms::computeParamsBSplineCurve(const Handle(TColgp_HArray1OfPnt)& points, Standard_Real umin, Standard_Real umax, const Standard_Real alpha)
{
    if ( umax <= umin ) {
        throw geoml::Error("The specified start parameter is larger than the specified end parameter");
    }
    
    std::vector<Standard_Real> parameters(static_cast<size_t>(points->Length()));

    parameters[0] = 0.;

    for (size_t i = 1; i < parameters.size(); ++i) {
        int iArray = static_cast<int>(i) + points->Lower();
        Standard_Real length = pow(points->Value(iArray).SquareDistance(points->Value(iArray - 1)), alpha / 2.);
        parameters[i] = parameters[i - 1] + length;
    }

    Standard_Real totalLength = parameters.back();


    for (size_t i = 0; i < parameters.size(); ++i) {
        Standard_Real ratio = 0.;
        if (totalLength < 1e-10) {
            ratio = static_cast<Standard_Real>(i) / static_cast<Standard_Real>(parameters.size()-1);
        }
        else {
...
BSplineAlgorithms::computeParamsBSplineSurf(const TColgp_Array2OfPnt& points, Standard_Real alpha)
{
    // first for parameters in u-direction:
    std::vector<Standard_Real> paramsU(static_cast<size_t>(points.ColLength()), 0.);
    for (int vIdx = points.LowerCol(); vIdx <= points.UpperCol(); ++vIdx) {
        std::vector<Standard_Real> parameters_u_line = computeParamsBSplineCurve(pntArray2GetColumn(points, vIdx), alpha);

        // average over columns
        for (size_t uIdx = 0; uIdx < parameters_u_line.size(); ++uIdx) {
            paramsU[uIdx] += parameters_u_line[uIdx]/(Standard_Real)points.RowLength();
        }
    }


    // now for parameters in v-direction:
    std::vector<Standard_Real> paramsV(static_cast<size_t>(points.RowLength()), 0.);
    for (int uIdx = points.LowerRow(); uIdx <= points.UpperRow(); ++uIdx) {
        std::vector<Standard_Real> parameters_v_line = computeParamsBSplineCurve(pntArray2GetRow(points, uIdx), alpha);

        // average over rows
        for (size_t vIdx = 0; vIdx < parameters_v_line.size(); ++vIdx) {
            paramsV[vIdx] += parameters_v_line[vIdx]/(Standard_Real)points.ColLength();
        }
    }
...
std::vector<Handle(Geom_BSplineCurve)> BSplineAlgorithms::createCommonKnotsVectorCurve(const std::vector<Handle(Geom_BSplineCurve)>& splines_vector, Standard_Real tol)
{
...
void BSplineAlgorithms::matchParameterRange(std::vector<Handle(Geom_BSplineCurve)> const& bsplines, Standard_Real tolerance)
{
    ...
    for (unsigned iP=1; iP<bsplines.size(); ++iP) {
        ...
        if (Abs(bspl->FirstParameter() - umin) > tolerance ||
            Abs(bspl->LastParameter() - umax) > tolerance ) {
...
ApproxResult BSplineAlgorithms::reparametrizeBSplineContinuouslyApprox(const Handle(Geom_BSplineCurve) spline,
                                                                                 const std::vector<Standard_Real>& old_parameters,
                                                                                 const std::vector<Standard_Real>& new_parameters,
                                                                                 size_t n_control_pnts)
{
    ...
    Standard_Real par_tol = 1e-10;

#define MODEL_KINKS
#ifdef MODEL_KINKS
    // remove kinks from breaks
    std::vector<Standard_Real> kinks = BSplineAlgorithms::getKinkParameters(spline);
    ...
    for (size_t ikink = 0; ikink < kinks.size(); ++ikink) {
        Standard_Real kink = kinks[ikink];
        std::vector<Standard_Real>::iterator it = std::find_if(breaks.begin(), breaks.end(), IsInsideTolerance(kink, par_tol));
        if (it != breaks.end()) {
            breaks.erase(it);
        }
    }
    ...
    std::vector<Standard_Real> parameters = LinspaceWithBreaks(new_parameters.front(),
                                                        new_parameters.back(),
    ...
    for (size_t i = 1; i <= parameters.size(); ++i) {
        Standard_Real oldParameter = reparametrizing_spline->Value(parameters[i-1]).X();
    ...
    for (size_t ibreak = 0; ibreak < breaks.size(); ++ibreak) {
        Standard_Real thebreak = breaks[ibreak];
                                                                    ...
                                                                    bool uContinuousIfClosed, bool vContinuousIfClosed)
{
    ...
    Standard_Real tolerance = REL_TOL_CLOSED * scale(points);
...
std::vector<std::pair<Standard_Real, Standard_Real> > BSplineAlgorithms::intersections(const Handle(Geom_BSplineCurve) spline1, const Handle(Geom_BSplineCurve) spline2, Standard_Real tolerance) {
    ...
    Standard_Real splines_scale = (BSplineAlgorithms::scale(spline1) + BSplineAlgorithms::scale(spline2)) / 2.;

    std::vector<std::pair<Standard_Real, Standard_Real> > intersection_params_vector;
...
Standard_Real BSplineAlgorithms::scale(const std::vector<Handle(Geom_BSplineCurve)>& splines_vector)
{
    Standard_Real maxScale = 0.;
...
Standard_Real BSplineAlgorithms::scale(const Handle(Geom_BSplineCurve)& spline)
{
    Standard_Real scale = 0.;
    ...
    for (int ctrl_pnt_idx = 2; ctrl_pnt_idx <= spline->NbPoles(); ++ctrl_pnt_idx) {
        ...
        Standard_Real distance = first_ctrl_pnt.Distance(spline->Pole(ctrl_pnt_idx));
...
ApproxResult BSplineAlgorithms::reparametrizeBSplineNiceKnots(Handle(Geom_BSplineCurve) spline)
{
    ...
    Standard_Real umin = spline->FirstParameter();
    Standard_Real umax = spline->LastParameter();
...
std::vector<Standard_Real> BSplineAlgorithms::getKinkParameters(const Handle(Geom_BSplineCurve)& curve)
{
    if (curve.IsNull()) {
        throw Error("Null Pointer curve", geoml::NULL_POINTER);
    }

    Standard_Real eps = 1e-8;

    std::vector<Standard_Real> kinks;
    for (int knotIndex = 2; knotIndex < curve->NbKnots(); ++knotIndex) {
        if (curve->Multiplicity(knotIndex) == curve->Degree()) {
            Standard_Real knot = curve->Knot(knotIndex);
            // check if really a kink
            Standard_Real angle = curve->DN(knot + eps, 1).Angle(curve->DN(knot - eps, 1));
            if (angle > 6./180. * M_PI) {
...
BSplineAlgorithms::SurfaceKinks BSplineAlgorithms::getKinkParameters(const Handle(Geom_BSplineSurface)& surface)
{
    ...
    for (int knotIndex = 2; knotIndex < surface->NbUKnots(); ++knotIndex) {
        if (surface->UMultiplicity(knotIndex) == surface->UDegree()) {
            Standard_Real knot = surface->UKnot(knotIndex);
    ...
    for (int knotIndex = 2; knotIndex < surface->NbVKnots(); ++knotIndex) {
        if (surface->VMultiplicity(knotIndex) == surface->VDegree()) {
            Standard_Real knot = surface->VKnot(knotIndex);
...
Handle(Geom_BSplineSurface) BSplineAlgorithms::trimSurface(const Handle(Geom_Surface)& surface, Standard_Real umin, Standard_Real umax, Standard_Real vmin, Standard_Real vmax)
{
    ...
    Standard_Real tol_check = 1e-12;
    auto intol = [tol_check](Standard_Real a, Standard_Real b) {
...
Handle(Geom_BSplineCurve) BSplineAlgorithms::trimCurve(const Handle(Geom_BSplineCurve)& curve, Standard_Real umin, Standard_Real umax)
{
...
Handle(Geom_BSplineCurve) BSplineAlgorithms::concatCurves(std::vector<Handle(Geom_BSplineCurve)> curves,
                                                               bool parByLength, Standard_Real tolerance)
{
    ...
    std::vector<Standard_Real> lengths;
    ...
    for (unsigned int icurve = 0; icurve < curves.size(); ++icurve) {
        Handle(Geom_BSplineCurve) curve = curves[icurve];
        Standard_Real stopPar = startPar;
        if (parByLength) {
