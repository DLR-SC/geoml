...
namespace
{
    ...
    geoml::FaceNameSettings GetFaceNameMode(const geoml::ExporterOptions& options)
    {
        return options.HasOption("FaceNames")
                ? options.Get<geoml::FaceNameSettings>("FaceNames")
                : geoml::IgesOptions().Get<geoml::FaceNameSettings>("FaceNames");
    }
    ...
    std::string ExportedLongFaceName(const std::string& shapeName, const std::string& faceName, geoml::FaceNameSettings settings)
    {
        switch (settings) {
        case geoml::UIDOnly:
            return shapeName;
        case geoml::FaceNameOnly:
            return faceName;
        case geoml::UIDandFaceName:
            return shapeName + "::" + faceName;
        case geoml::None:
            return "";
        }
        
        throw geoml::Error("Unrecognized FaceNameSettings");
    }

    /**
     * @brief WriteIGESFaceNames takes the names of each face and writes it into the IGES model.
     */
    void WriteIGESFaceNames(IGESControl_Writer& writer, const PNamedShape shape, const geoml::ExporterOptions& globalOptions, int level)
    {
        if (!shape) {
            return;
        }
    ...
    void WriteIgesNames(IGESControl_Writer& writer, const PNamedShape shape, const geoml::ExporterOptions& globalOptions, int level)
    {
        WriteIGESFaceNames(writer, shape, globalOptions, level);
        WriteIGESShapeNames(writer, shape, level);
    }

    int GetBRepMode(const geoml::ExportIges& writer)
    {
        return writer.GlobalExportOptions().HasOption("IGES5.3")
                ? writer.GlobalExportOptions().Get<bool>("IGES5.3")
                : geoml::IgesOptions().Get<bool>("IGES5.3");
    }
...
AUTORUN(ExportIges)
{
    static CCADExporterBuilder<ExportIges> igesExporterBuilder;
        ExporterFactory::Instance().RegisterExporter(&igesExporterBuilder, IgesOptions());
    return true;
}

// Constructor
ExportIges::ExportIges(const ExporterOptions& opt)
    : CADExporter(opt)
{
}

ExporterOptions ExportIges::GetDefaultOptions() const
{
    return IgesOptions();
}

ShapeExportOptions ExportIges::GetDefaultShapeOptions() const
{
    return IgesShapeOptions();
}

void ExportIges::SetTranslationParameters() const
{
    ...
    Interface_Static::SetCVal("write.iges.header.author", "geoml");
...
bool ExportIges::WriteImpl(const std::string& filename) const
{
    if (filename.empty()) {
       LOG(ERROR) << "Error: Empty filename in ExportIges::Write.";
       return false;
    }
 ...
 */
void ExportIges::AddToIges(PNamedShape shape, IGESControl_Writer& writer, int level) const
{
    if (!shape) {
        return;
    }
    ...
    if (faceMap.Extent() > 0) {
        ...
        if (ret > IFSelect_RetDone) {
            throw Error("Export to IGES file failed in ExportIges. Could not translate shape "
                             + shapeName + " to iges entity,", geoml::GENERIC_ERROR);
        }
