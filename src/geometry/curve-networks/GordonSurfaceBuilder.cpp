                                                    ...
                                                    const std::vector<Standard_Real>& intersection_params_spline_v)
{
    ...
    if (profiles.size() < 2) {
        throw Error("There must be at least two profiles for the gordon surface.", geoml::MATH_ERROR);
    }

    if (guides.size()  < 2) {
        throw Error("There must be at least two guides for the gordon surface.", geoml::MATH_ERROR);
    }
