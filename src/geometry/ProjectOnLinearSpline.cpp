...
ProjectOnLinearSpline::ProjectOnLinearSpline(Handle(Geom_BSplineCurve) curve, gp_Pnt p)
    : _curve(curve), _isDone(false), _point(p)
{
}

bool ProjectOnLinearSpline::IsDone()
{
    compute();
...
gp_Pnt ProjectOnLinearSpline::Point()
{
    if (!IsDone()) {
        throw Error("Can not compute proejection point in ProjectOnLinearSpline::Point", geoml::MATH_ERROR);
    }
...
ProjectOnLinearSpline::~ProjectOnLinearSpline()
{
}

bool ProjectOnLinearSpline::compute()
{
    if (_isDone) {
        return true;
    }
