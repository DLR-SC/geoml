...
AUTORUN(ImportStep)
{
    static ICADImporterCreatorImpl<ImportStep> stepImporterCreator;
        ImporterFactory::Instance().RegisterImporter(&stepImporterCreator);
    return true;
}
...
ListPNamedShape ImportStep::Read(const std::string stepFileName)
{
    ...
    if ( status == IFSelect_RetDone ) {
        ...
        throw Error( "Cannot read step file " + stepFileName + "!", geoml::OPEN_FAILED);
    }
...
std::string geoml::ImportStep::SupportedFileType() const
{
    return "step";
}
