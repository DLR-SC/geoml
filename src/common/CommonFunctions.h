...
GEOML_EXPORT Standard_Real GetLength(const TopoDS_Wire& wire);
...
GEOML_EXPORT gp_Pnt GetFirstPoint(const TopoDS_Wire& w);
GEOML_EXPORT gp_Pnt GetFirstPoint(const TopoDS_Edge& e);

// returns the end point of the wire/edge
GEOML_EXPORT gp_Pnt GetLastPoint(const TopoDS_Shape& wireOrEdge);
...
GEOML_EXPORT gp_Pnt ProjectPointOnShape(const TopoDS_Shape& shape, const gp_Pnt& point, const gp_Vec& direction);
 ...
 * @return a std::optional<UVResult> instance containing the face together with the
 ...
 */
GEOML_EXPORT TopoDS_Shape ReplaceFaceInShape(TopoDS_Shape const& shape,
                                            TopoDS_Face const& new_face,
                                            TopoDS_Face const& old_face);

// checks, whether a face is in between two points
GEOML_EXPORT bool IsFaceBetweenPoints(const TopoDS_Face& face, gp_Pnt p1, gp_Pnt p2);
...
GEOML_EXPORT IntStatus IntersectLinePlane(gp_Pnt p1, gp_Pnt p2, gp_Pln plane, gp_Pnt& result);

// returns the number of edges of the current shape
GEOML_EXPORT unsigned int GetNumberOfEdges(const TopoDS_Shape& shape);

// returns the number of faces of the current shape
GEOML_EXPORT unsigned int GetNumberOfFaces(const TopoDS_Shape& shape);

GEOML_EXPORT TopoDS_Edge GetEdge(const TopoDS_Shape& shape, int iEdge);

GEOML_EXPORT TopoDS_Face GetFace(const TopoDS_Shape& shape, int iFace);

GEOML_EXPORT Handle(Geom_BSplineCurve) GetBSplineCurve(const TopoDS_Edge& e);

// Returns the number of subshapes, if the shape is a compound
GEOML_EXPORT unsigned int GetNumberOfSubshapes(const TopoDS_Shape& shape);

// returns the central point of the face
GEOML_EXPORT gp_Pnt GetCentralFacePoint(const class TopoDS_Face& face);

// puts all faces with the same origin to one TopoDS_Compound
// Maps all compounds with its name in the map
GEOML_EXPORT ListPNamedShape GroupFaces(const PNamedShape shape, geoml::ShapeGroupMode groupType);

GEOML_EXPORT TopoDS_Shape GetFacesByName(const PNamedShape shape, const std::string& name);
...
GEOML_EXPORT bool IsPointInsideShape(const TopoDS_Shape& solid, gp_Pnt point, Bnd_Box const* bounding_box = nullptr);

// Checks, whether a point lies inside a given face
GEOML_EXPORT bool IsPointInsideFace(const TopoDS_Face& face, gp_Pnt point);

// Checks whether a point lies above or below a plane (determined by direction of normal)
GEOML_EXPORT bool IsPointAbovePlane(const gp_Pln& pln, gp_Pnt point);
...
GEOML_EXPORT bool IsPathRelative(const std::string&);

// Returns true, if a file is readable
GEOML_EXPORT bool IsFileReadable(const std::string& filename);

GEOML_EXPORT std::string FileExtension(const std::string& filename);

// get the continuity of two edges which share a common vertex
GEOML_EXPORT geoml::Continuity getEdgeContinuity(const TopoDS_Edge& edge1, const TopoDS_Edge& edge2);

GEOML_EXPORT Standard_Boolean IsEqual(const TopoDS_Shape& s1, const TopoDS_Shape& s2);

// Method for building a face out of two wires
GEOML_EXPORT TopoDS_Face BuildFace(const TopoDS_Wire& wire1, const TopoDS_Wire& wire2);

// Method for building a face out of two wires and a direction vector
GEOML_EXPORT TopoDS_Face BuildFace(const TopoDS_Wire& wire1, const TopoDS_Wire& wire2, const gp_Vec& dir);
...
GEOML_EXPORT TopoDS_Face BuildFace(const TopoDS_Wire& wire);
...
GEOML_EXPORT TopoDS_Face BuildRuledFace(const TopoDS_Wire& wire1, const TopoDS_Wire& wire2);

// Method for building a wire out of two points
GEOML_EXPORT TopoDS_Wire BuildWire(const gp_Pnt& p1, const gp_Pnt& p2);

// Method for building a wire out of the edges from the passed geometry
GEOML_EXPORT TopoDS_Wire BuildWireFromEdges(const TopoDS_Shape& edges);

// Returns a list of wires built from all connected edges in the passed shape
GEOML_EXPORT void BuildWiresFromConnectedEdges(const TopoDS_Shape& shape, TopTools_ListOfShape& wireList);

// Method for creating a face from an opened wire
GEOML_EXPORT TopoDS_Wire CloseWire(const TopoDS_Wire& wire);
...
GEOML_EXPORT TopoDS_Wire CloseWires(const TopoDS_Wire& wire1, const TopoDS_Wire& wire2);

// Method for closing two wires to a single one, 
// the passed vector is used to define the upper and lower end vertices of the wires
GEOML_EXPORT TopoDS_Wire CloseWires(const TopoDS_Wire& wire1, const TopoDS_Wire& wire2, const gp_Vec& dir);

// Method for sorting the edges of a wire
GEOML_EXPORT TopoDS_Wire SortWireEdges(const TopoDS_Wire& wire);

// Returns the first and last vertex of the passed shape along the passed 
// direction
GEOML_EXPORT bool GetMinMaxPoint(const TopoDS_Shape& shape, const gp_Vec& dir, gp_Pnt& minPnt, gp_Pnt& maxPnt);

// Returns the list of shapes of the passed type from the passed shape
GEOML_EXPORT void GetListOfShape(const TopoDS_Shape& shape, TopAbs_ShapeEnum type, TopTools_ListOfShape& result);

// Returns all shapes with the given type contained in the given shape
GEOML_EXPORT std::vector<TopoDS_Shape> GetSubShapes(const TopoDS_Shape& shape, TopAbs_ShapeEnum type);
...
GEOML_EXPORT TopoDS_Shape CutShapes(const TopoDS_Shape& shape1, const TopoDS_Shape& shape2);

// Helper for splitting a shape by another shape
GEOML_EXPORT TopoDS_Shape SplitShape(const TopoDS_Shape& src, const TopoDS_Shape& tool);
...
GEOML_EXPORT void FindAllConnectedEdges(const TopoDS_Edge& edge, TopTools_ListOfShape& edgeList, TopTools_ListOfShape& targetList);

// Method for checking if two edges have a common vertex (same position)
GEOML_EXPORT bool CheckCommonVertex(const TopoDS_Edge& e1, const TopoDS_Edge& e2);

// Method for searching all vertices which are only connected to a single edge
GEOML_EXPORT void GetEndVertices(const TopoDS_Shape& shape, TopTools_ListOfShape& endVertices);

// Method for finding the face which has the lowest distance to the passed point
GEOML_EXPORT TopoDS_Face GetNearestFace(const TopoDS_Shape& src, const gp_Pnt& pnt);
