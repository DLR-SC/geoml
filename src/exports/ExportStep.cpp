...
AUTORUN(ExportStep)
{
        static CCADExporterBuilder<ExportStep> stepExporterBuilder;
        ExporterFactory::Instance().RegisterExporter(&stepExporterBuilder, StepOptions());
    return true;
}

// Constructor
ExportStep::ExportStep(const ExporterOptions& opt)
    : CADExporter(opt)
{
}

ExporterOptions ExportStep::GetDefaultOptions() const
{
    return StepOptions();
}

ShapeExportOptions ExportStep::GetDefaultShapeOptions() const
{
 ...
 */
void ExportStep::AddToStep(PNamedShape shape, STEPControl_Writer& writer) const 
{
    if (!shape) {
        return;
    }
    ...
    if (faceMap.Extent() > 0) {
        ...
        if (ret > IFSelect_RetDone) {
                throw Error("Export to STEP file failed in ExportStep. Could not translate shape "
                                + shapeName + " to step entity,", geoml::GENERIC_ERROR);
        }
...
bool ExportStep::WriteImpl(const std::string& filename) const
{
    if ( filename.empty()) {
       LOG(ERROR) << "Error: Empty filename in ExportStep::Write.";
       return false;
    }
