    ...
    , m_C2Continuous(continuousIfClosed)
{
    ...
    if (points.IsNull()) {
        throw Error("No points given in PointsToBSplineInterpolation", geoml::NULL_POINTER);
    }

    if (points->Length() < 2) {
        throw Error("Too few points in PointsToBSplineInterpolation", geoml::MATH_ERROR);
    }
}
